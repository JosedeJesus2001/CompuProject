...
int Opcion1=0;
char continuaroNo='S';
int numeroFuente=0, resistenciaFinal=0,resistenciaInicial=0,configuracionDada;
float voltajeEntrada;
char terminalInversora='x', terminalNoInversora='x',opcionesTerminales='x';
...
int main() {
    while (continuaroNo == 'S') {
        ...
        if (Opcion1 == 4) {
            ...
            cin>>opcionOpAmp;
            if (opcionOpAmp=='C'){
            char opcionTerminalInversora;
             cout<<"Ingresa el número de fuentes en la configuración"<<endl;
             cin>>numeroFuente;
                if (numeroFuente==1){
                    cout<<"¿Que está conectado en la entrada inversora del OpAmp?"<<endl;
                    cout<<"Fuente[F]| Tierra[T]"<<endl;
                    cin>>opcionTerminalInversora;
                    if (opcionTerminalInversora=='F'){
                        cout<<"La configuración del OpAmp es 'Inversora'"<<endl;
                    } else{
                        cout<<"La configuración del OpAmp es 'No Inversora'"<<endl;
                    }
                } else{
                    cout<<"Las fuentes están conectadas en una terminal[U] o en ambas[A]"<<endl;
                    cin>>opcionesTerminales;
                    if (opcionesTerminales=='U'){
                        cout<<"La configuración está en amplificador sumador"<<endl;
                    } else{
                        cout<<"La configuración está en amplificador de diferencia"<<endl;
                    }
                }
            }
            if (opcionOpAmp=='P'){
                cout<<"¿Qué configuración desea conocer?"<<endl;
                cout<<"Inversora[1]| No inversora[2]| Sumadora[3]| Diferencia[4]"<<endl;
                cin>>configuracionDada;
                if (configuracionDada==1){
                    cout<<"La fuente está conectada a la entrada inversora"<<endl;
                    cout<<"La tierra está conectada a la entrada no inversora"<<endl;
                }
                if (configuracionDada==2){
                    cout<<"La fuente está conectada a la entrada no inversora"<<endl;
                    cout<<"La tierra está conectada a la entrada inversora"<<endl;
                }
                if (configuracionDada==3){
                    cout<<"Las fuentes están conectadas a la entrada inversora"<<endl;
                    cout<<"La tierra está conectada a la entrada no inversora"<<endl;
                }
                if (configuracionDada==4){
                    cout<<"Ambas terminales están conectadas a una fuente"<<endl;
                }
            }
            if (opcionOpAmp=='V'){
                cout<<"Ingresa la configuración para calcular Vo"<<endl;
                cout<<"Inversora[1]| No inversora[2]| Sumadora[3]| Diferencia[4]"<<endl;
                cin>>configuracionDada;
                cout<<"Ingrese Vo"<<endl;
                cin>>voltajeEntrada;
                cout<<"Ingresa la resistencia inicial y final"<<endl;
                cout<<"Resistencia inicial:"<<endl;
                cin>>resistenciaInicial;
                cout<<"Resistencia final"<<endl;
                cin>>resistenciaFinal;
                if (configuracionDada==1){
                    cout<<"Vo es igual a: "<<((resistenciaFinal/resistenciaInicial)*voltajeEntrada*-1)<<endl;
                }
                if (configuracionDada==2){
                    cout<<"Vo es igual a: "<<(1+(resistenciaFinal/resistenciaInicial))*voltajeEntrada<<endl;
                }
                if (configuracionDada==3){
                    int numeroFuentesOpAmpSumador;
                    float sumaVoltajesOpAmpSumador;
                    cout<<"Ingrese la cantidad de resistencias que vas a sumar en serie"<<endl;
                    cin>>numeroFuentesOpAmpSumador;
                    vector<float> vectorFuentesOpAmpSumador;
                    float valorFuentes;
                    for (int i = 0; i <numeroFuentesOpAmpSumador ; ++i) {
                        cout<<"Ingresa el valor de la fuente"<< i+1<<" en Volts"<<endl;
                        cin>>valorFuentes;
                        vectorFuentesOpAmpSumador.push_back(valorFuentes);
                        sumaVoltajesOpAmpSumador+=vectorFuentesOpAmpSumador[i];
                    }
                    cout<<"Vo es igual a: "<<((sumaVoltajesOpAmpSumador)*(resistenciaFinal/resistenciaInicial)*(-1))<<endl;
                }
                if (configuracionDada==4){
                    float voltajeInversora, voltajeNoInversora;
                    cout<<"Ingrese su voltaje conectado a la entrada inversora"<<endl;
                    cin>>voltajeInversora;
                    cout<<"Ingrese su voltaje conectado a la entrada no inversora"<<endl;
                    cin>>voltajeNoInversora;
                    cout<<"Vo es igual a: "<<voltajeNoInversora-voltajeInversora<<endl;
                }
            }
        }
        if (Opcion1 > 4) {
